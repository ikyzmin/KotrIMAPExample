...
fun main() {
    runBlocking {
        ...
        w.write("$TAG${tagCounter++} UID SORT RETURN (SAVE COUNT) (DATE) UTF-8 OR OR OR OR FROM \"test\" SUBJECT \"test\" TO \"test\" CC \"test\" BCC \"test\" \r\n")
        w.write("$TAG${tagCounter++} UID FETCH $ (UID BODYSTRUCTURE BODY.PEEK[4]) \r\n")
        w.write("$TAG${tagCounter++} UID SORT RETURN (SAVE) (DATE) UTF-8 NOT UID $ NOT FROM \"test\" NOT SUBJECT \"test\" NOT TO \"test\" NOT CC \"test\" NOT BCC \"test\" BODY \"test\" \r\n")
        w.write("$TAG${tagCounter++} UID FETCH $ (UID) \r\n")
        w.flush()
